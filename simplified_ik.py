...
class IK:
    ...
    def inverse(self, target, seed):
        ...
        for step in range(self.max_steps):
            ...
            W = np.diag([1.0, 1.2, 1.0, 0.8, 1.0, 0.8, 0.6])
            H = W @ W.T + 1e-3 * np.eye(n_joints)
            ...
            dq_max = 0.2 * np.ones(n_joints)  # Increased max velocity
            ...
            try:
                ...
                print(f"QP solver failed at iteration {step}, using DLS fallback")
                lambda_ = 0.5
                dq = np.linalg.solve(J.T @ J + lambda_ * np.eye(n_joints), J.T @ error)
