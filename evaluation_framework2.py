...
from dls_ik_solver3 import IK as DLSolver
...
class ComparativeAnalysis:
    def run_comparison(self, qp_solver, sdls_solver, dls_solver, test_cases):
        results = {
            'singularity_handling': self.compare_singularity_handling(qp_solver, sdls_solver, dls_solver, test_cases['near_singularity']),
            'joint_limits': self.compare_joint_limits(qp_solver, sdls_solver, dls_solver, test_cases['joint_limits']),
            'convergence': self.compare_convergence(qp_solver, sdls_solver, dls_solver, test_cases['standard_poses']),
            'trajectory_tracking': self.compare_trajectory_tracking(qp_solver, sdls_solver, dls_solver, test_cases['dynamic_tracking'])
        }
        return results
        
    def compare_singularity_handling(self, qp_solver, sdls_solver, dls_solver, singular_poses):
        qp_metrics = []
        sdls_metrics = []
        dls_metrics = []
...
class IKBenchmark:
    def __init__(self):
        # Initialize solvers with same parameters
...
class ResultVisualizer:
    def plot_comparison(self, qp_results: BenchmarkResults, sdls_results: BenchmarkResults, 
                       dls_results: BenchmarkResults, test_type: str):
        plt.figure(figsize=(15, 10))
        
        # Success rate comparison
        plt.subplot(231)
        self.plot_bar_comparison(['QP', 'SDLS', 'DLS'],
                               [qp_results.success_rate, sdls_results.success_rate, 
                                dls_results.success_rate],
        ...
        self.plot_bar_comparison(['QP', 'SDLS', 'DLS'],
                               [qp_results.convergence_time, sdls_results.convergence_time,
                                dls_results.convergence_time],
                               'Average Convergence Time (s)', test_type)
        
        # Error comparison
        plt.subplot(233)
        self.plot_error_comparison(qp_results, sdls_results, dls_results, test_type)
        
        # Manipulability comparison
        plt.subplot(234)
        self.plot_bar_comparison(['QP', 'SDLS', 'DLS'],
                               [qp_results.manipulability, sdls_results.manipulability,
                                dls_results.manipulability],
                               'Average Manipulability Index', test_type)
        
        # Joint velocity profiles
        plt.subplot(235)
        self.plot_velocity_profiles(qp_results.joint_velocities, 
                                  sdls_results.joint_velocities,
                                  dls_results.joint_velocities)
        
        # Smoothness comparison
        plt.subplot(236)
        self.plot_bar_comparison(['QP', 'SDLS', 'DLS'],
                               [qp_results.smoothness, sdls_results.smoothness,
                                dls_results.smoothness],
    ...
    def plot_error_comparison(self, qp_results: BenchmarkResults, sdls_results: BenchmarkResults, 
                            dls_results: BenchmarkResults, test_type: str):
        ...
        dls_errors = [dls_results.position_error, dls_results.orientation_error]
        
        x = np.arange(len(labels))
        width = 0.25
        
        plt.bar(x - width, qp_errors, width, label='QP')
        plt.bar(x, sdls_errors, width, label='SDLS')
        plt.bar(x + width, dls_errors, width, label='DLS')
...
def main():
    ...
    for category, method_name in category_methods.items():
        ...
        try:
            ...
            dls_results = benchmark.run_solver_tests(benchmark.dls_solver, test_cases)
